    ...
    while (!done)
    {
        ...
        *p_open = false;
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
        {
            ...
            ImGui::Begin(name_app,p_open, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);                          // Create a window called "Hello, world!" and append into it.

            


            const char* combo_preview_value = translationsList.data()[item_current];
            ...
            if (ImGui::Button(button_create))
                ImGui::OpenPopup(button_create);

            ImGui::SameLine();
            if (ImGui::Button(refresh))
                can_update = true;
            ImGui::SameLine();
            if (ImGui::Button(button_search)) {
                ImGui::OpenPopup(button_search);
            }
            ...
            {
                if (ImGui::BeginTabItem(check_catalogue))
                {
                    setTableClip(libCards);
                    ImGui::EndTabItem();
                }
                if (isdebug == true) {
